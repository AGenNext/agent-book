export const ruRU = {
  ...
  connectionErrors: {
    ...
    apiDesc: "API-сервер Autonomyx AgentBook недоступен",
    ...
    docLink: "Документация Autonomyx AgentBook",
  ...
  navigation: {
    ...
    groundedChat: "Чат с опорой на источники",
    artifacts: "Студия артефактов",
  ...
  workspace: {
    ...
    summaryCards: {
      ...
      notes: "Заметки знаний",
      groundedCoverage: "Покрытие доказательствами",
      openActions: "Открытые действия",
    },
    pinnedKnowledge: "Закреплённые знания",
    pinnedKnowledgeDesc: "Надёжные, проверенные и готовые к повторному использованию канонические утверждения.",
    samplePinned: {
      claim1: "Утверждения без прямых цитат блокируются перед публикацией.",
      claim2: "Все сводки по политикам требуют хотя бы один актуальный источник за последние 90 дней.",
    },
    activityTitle: "Последняя активность",
    activityDesc: "Аудируемые события для этого рабочего пространства и его участников.",
    timelineDesc: "Непрерывная память о доказательствах, решениях и результатах.",
    tasksDesc: "Преобразуйте подтверждённые инсайты в ответственных, сроки и согласования.",
    membersDesc: "Ролевое сотрудничество для надёжных корпоративных процессов.",
    settingsDesc: "Средства управления для ответов с опорой на источники, цитирования и отслеживаемости.",
    artifactDesc: "Создавайте переиспользуемые материалы на основе источников с сохранением цитат.",
    artifactPlaceholder: "Каркас генерации артефактов включён. Подключите workflow оркестрации к backend-шаблонам, чтобы выпускать результаты со связанными цитатами.",
    generateArtifact: "Сгенерировать артефакт",
    duplicate: "Дублировать рабочее пространство",
    activity: {
      sourceAdded: "Новый источник добавлен в {workspace} и поставлен в очередь на индексацию для поиска.",
      sourceAddedGeneric: "Загрузка источника завершена, и доказательства теперь доступны в области чата.",
      noteCaptured: "Заметка с подтверждением доказательствами сохранена и связана с ссылками на источник.",
      answerSaved: "Ответ с опорой на источники сохранён как переиспользуемое знание рабочего пространства.",
      taskCreated: "Создана задача на основе подтверждённого утверждения.",
    },
    sampleTasks: {
      task1: "Проверить нерешённые утверждения с недостаточными доказательствами",
      task2: "Утвердить executive brief для распространения среди заинтересованных сторон",
    },
    settings: {
      audit: "Журнал аудита обязателен для всех существенных действий",
      insufficientEvidence: "Ответы с недостаточными доказательствами видны всем участникам",
      citationPolicy: "Проверка цитат остаётся включённой для сгенерированных материалов",
    },
  },
  sources: {
    title: "Источники",
  ...
  settings: {
    ...
    filesHelp: "После загрузки и обработки файлы больше не нужны. Большинству пользователей следует разрешить Autonomyx AgentBook автоматически удалять загруженные файлы из папки загрузок.",
  ...
  advanced: {
    ...
    updateAvailableDesc: "Доступна новая версия Autonomyx AgentBook.",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "Настройте, какие модели использовать для различных задач в Autonomyx AgentBook",
    missingRequiredModels: "Отсутствуют необходимые модели: {models}. Autonomyx AgentBook может работать некорректно без них.",
  ...
  apiKeys: {
    ...
    description: "Храните API-ключи в базе данных для безопасного подключения провайдеров ИИ в Autonomyx AgentBook.",
