export const jaJP = {
  ...
  connectionErrors: {
    ...
    apiDesc: "Autonomyx AgentBook APIサーバーに到達できませんでした",
    ...
    docLink: "Autonomyx AgentBookドキュメント",
  ...
  navigation: {
    ...
    notebooks: "ノートブック",
    workspaces: "ワークスペース",
    groundedChat: "根拠付きチャット",
    artifacts: "アーティファクトスタジオ",
    askAndSearch: "質問と検索",
    models: "モデル",
  ...
  workspace: {
    ...
    summaryCards: {
      sources: "接続済みソース",
      notes: "ナレッジノート",
      groundedCoverage: "根拠カバレッジ",
      openActions: "未対応アクション",
    },
    pinnedKnowledge: "固定ナレッジ",
    pinnedKnowledgeDesc: "信頼でき、レビュー済みで再利用可能な正規の主張です。",
    samplePinned: {
      claim1: "直接引用のない主張は公開できません。",
      claim2: "すべてのポリシー要約には過去90日以内の最新ソースが最低1件必要です。",
    },
    activityTitle: "最近のアクティビティ",
    activityDesc: "このワークスペースとメンバーの監査可能なイベントです。",
    timelineDesc: "証拠、判断、結果の継続的な記録です。",
    tasksDesc: "検証済みの洞察を担当者、期限、承認に変換します。",
    membersDesc: "信頼できるエンタープライズワークフロー向けのロールベース協業です。",
    settingsDesc: "根拠付き回答、引用、追跡可能性のためのガバナンス制御です。",
    artifactDesc: "引用を保持したまま再利用可能なソース根拠付き成果物を生成します。",
    artifactPlaceholder: "アーティファクト生成の足場が有効です。オーケストレーションワークフローをバックエンドテンプレートに接続して、引用リンク付きの出力を生成します。",
    generateArtifact: "アーティファクトを生成",
    duplicate: "ワークスペースを複製",
    activity: {
      sourceAdded: "{workspace} に新しいソースが追加され、検索インデックス化のキューに入りました。",
      sourceAddedGeneric: "ソース取り込みが完了し、証拠をチャット範囲で利用できます。",
      noteCaptured: "証拠に基づくノートが取得され、ソース参照にリンクされました。",
      answerSaved: "根拠付き回答が再利用可能なワークスペース知識として保存されました。",
      taskCreated: "検証済みの主張からアクションタスクが作成されました。",
    },
    sampleTasks: {
      task1: "証拠不足の未解決主張を確認する",
      task2: "関係者配布用のエグゼクティブブリーフを承認する",
    },
    settings: {
      audit: "すべての重要操作で監査ログが必須です",
      insufficientEvidence: "証拠不足の回答はすべてのメンバーに表示されます",
      citationPolicy: "生成出力では引用確認が有効のままです",
    },
  },
  sources: {
    title: "ソース",
  ...
  settings: {
    ...
    filesHelp: "ファイルがアップロードされて処理されると、ファイル自体は不要になります。ほとんどのユーザーはAutonomyx AgentBookがアップロードフォルダから自動的にファイルを削除することを許可すべきです。",
  ...
  advanced: {
    ...
    updateAvailableDesc: "Autonomyx AgentBookの新しいバージョンが利用可能です。",
    upToDate: "最新版です",
    unknown: "不明",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "Autonomyx AgentBook全体で異なる目的に使用するモデルを設定",
    missingRequiredModels: "必須モデルがありません: {models}。これらがないとAutonomyx AgentBookが正しく機能しない可能性があります。",
  ...
  apiKeys: {
    ...
    description: "APIキーをデータベースに安全に保存し、Autonomyx AgentBookでAIプロバイダーを有効にします。",
