export const zhCN = {
  ...
  connectionErrors: {
    ...
    apiDesc: "无法访问 Autonomyx AgentBook API 服务器",
    dbTitle: "数据库连接失败",
    ...
    docLink: "Autonomyx AgentBook 文档",
  ...
  workspace: {
    ...
    summaryCards: {
      sources: "已连接来源",
      notes: "知识笔记",
      groundedCoverage: "证据覆盖率",
      openActions: "待处理事项",
    },
    pinnedKnowledge: "置顶知识",
    pinnedKnowledgeDesc: "可信、已审阅且可重复使用的规范性结论。",
    samplePinned: {
      claim1: "没有直接引用的结论将被阻止发布。",
      claim2: "所有策略摘要都至少需要一条最近 90 天内的当前来源。",
    },
    activityTitle: "最近活动",
    activityDesc: "此工作区及其成员的可审计事件。",
    timelineDesc: "关于证据、决策和结果的连续记录。",
    tasksDesc: "将已验证的洞察转化为负责人、截止日期和审批。",
    membersDesc: "面向可信企业工作流的基于角色协作。",
    settingsDesc: "针对有据可依回答、引用和可追溯性的治理控制。",
    artifactDesc: "生成可复用且保留引用的基于来源的输出。",
    artifactPlaceholder: "制品生成框架已启用。将编排工作流连接到后端模板以生成带引用链接的输出。",
    generateArtifact: "生成制品",
    duplicate: "复制工作区",
    activity: {
      sourceAdded: "新来源已添加到 {workspace} 并排队进行检索索引。",
      sourceAddedGeneric: "来源摄取已完成，证据现已可在聊天范围内使用。",
      noteCaptured: "已捕获基于证据的笔记并关联到来源引用。",
      answerSaved: "基于证据的回答已保存为可复用的工作区知识。",
      taskCreated: "已从已验证结论创建行动任务。",
    },
    sampleTasks: {
      task1: "审查证据不足的未解决结论",
      task2: "批准面向利益相关者分发的高管简报",
    },
    settings: {
      audit: "所有重要操作都必须启用审计日志",
      insufficientEvidence: "证据不足的回答对所有成员可见",
      citationPolicy: "生成输出时仍启用引用检查",
    },
  },
  sources: {
    title: "来源",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "配置用于 Autonomyx AgentBook 不同用途的默认模型",
    missingRequiredModels: "缺少必需的模型：{models}。如果没有这些模型，Autonomyx AgentBook 可能无法正常运行。",
  ...
  apiKeys: {
    ...
    description: "将 API 密钥安全地存储在数据库中，以在 Autonomyx AgentBook 中启用 AI 服务商。",
