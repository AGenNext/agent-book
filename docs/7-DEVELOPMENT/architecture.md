...
AgentBook follows a three-tier architecture with clear separation of concerns:
...
LangGraph is a state machine library that orchestrates multi-step AI workflows. AgentBook uses five core workflows:

### 1. **Source Processing Workflow** (`agentbook/graphs/source.py`)
...
### 2. **Chat Workflow** (`agentbook/graphs/chat.py`)
...
### 3. **Ask Workflow** (`agentbook/graphs/ask.py`)
...
### 4. **Transformation Workflow** (`agentbook/graphs/transformation.py`)
...
### 5. **Prompt Workflow** (`agentbook/graphs/prompt.py`)
...
Located in `agentbook/ai/models.py`, ModelManager handles:
...
**Usage**:
```python
from agentbook.ai.provision import provision_langchain_model
...
**Domain Objects** (`agentbook/domain/`):
...
**Repository Pattern**:
- Database access layer (`agentbook/database/repository.py`)
...
1. Create `agentbook/graphs/workflow_name.py`
...
1. Create model in `agentbook/domain/model_name.py`
