# Autonomyx AgentBook Architecture

## High-Level Overview

Autonomyx AgentBook follows a three-tier architecture with clear separation of concerns:
...
LangGraph is a state machine library that orchestrates multi-step AI workflows. Autonomyx AgentBook uses five core workflows:

### 1. **Source Processing Workflow** (`autonomyx_agentbook/graphs/source.py`)
...
### 2. **Chat Workflow** (`autonomyx_agentbook/graphs/chat.py`)
...
### 3. **Ask Workflow** (`autonomyx_agentbook/graphs/ask.py`)
...
### 4. **Transformation Workflow** (`autonomyx_agentbook/graphs/transformation.py`)
...
### 5. **Prompt Workflow** (`autonomyx_agentbook/graphs/prompt.py`)
...
Located in `autonomyx_agentbook/ai/models.py`, ModelManager handles:
...
**Usage**:
```python
from autonomyx_agentbook.ai.provision import provision_langchain_model
...
**Domain Objects** (`autonomyx_agentbook/domain/`):
...
**Repository Pattern**:
- Database access layer (`autonomyx_agentbook/database/repository.py`)
...
1. Create `autonomyx_agentbook/graphs/workflow_name.py`
...
1. Create model in `autonomyx_agentbook/domain/model_name.py`
