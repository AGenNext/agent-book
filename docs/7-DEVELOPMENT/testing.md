...
This document provides guidelines for writing tests in Autonomyx AgentBook. Testing is critical to maintaining code quality and preventing regressions.
...
from autonomyx_agentbook.domain.notebook import Notebook
...
```bash
uv run pytest --cov=autonomyx_agentbook
```
...
- Use `--cov` flag to check coverage: `uv run pytest --cov=autonomyx_agentbook`
