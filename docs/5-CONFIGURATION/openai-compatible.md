...
Use any server that implements the OpenAI API format with Autonomyx AgentBook. This includes LM Studio, Text Generation WebUI, vLLM, and many others.
...
Autonomyx AgentBook can connect to any server using this format.
...
### Step 3: Add Model in Autonomyx AgentBook
...
When Autonomyx AgentBook runs in Docker and your compatible server runs on the host, use the appropriate base URL when adding your credential in **Settings → API Keys**:
...
## Adding Models in Autonomyx AgentBook
...
```bash
docker exec -it autonomyx-agentbook curl http://host.docker.internal:1234/v1/models
```
...
4. Update model name in Autonomyx AgentBook
```
