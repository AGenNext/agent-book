...
Deploy Autonomyx AgentBook behind nginx, Caddy, Traefik, or other reverse proxies with custom domains and HTTPS.

---

## Simplified Setup (v1.1+)

Starting with v1.1, Autonomyx AgentBook uses Next.js rewrites to simplify configuration. **You only need to proxy to one port** - Next.js handles internal API routing automatically.
...
server {
    ...
    location / {
        proxy_pass http://autonomyx-agentbook:8502;
...
notebook.example.com {
    reverse_proxy autonomyx-agentbook:8502 {
        transport http {
            read_timeout 600s
            write_timeout 600s
        }
...
# See full base setup: ./docker-compose.yml
services:
  autonomyx-agentbook:
...
> **Note:** This example only shows the autonomyx-agentbook and nginx services. You also need a `surrealdb` service. See the [full base docker-compose.yml](./docker-compose.yml) for the complete setup.
...
services:
  autonomyx-agentbook:
    image: autonomyx/agentbook:v1-latest
    pull_policy: always
    container_name: autonomyx-agentbook
    environment:
      - API_URL=https://notebook.example.com
...
http {
    upstream notebook {
        server autonomyx-agentbook:8502;
    }
...
location /api/ {
    proxy_pass http://autonomyx-agentbook:5055/api/;
    proxy_http_version 1.1;
    proxy_set_header Host $host;
    proxy_set_header X-Real-IP $remote_addr;
    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
...
Accessing Autonomyx AgentBook from a different machine on your network:

**Step 1: Get your server IP**
```bash
# On the server running Autonomyx AgentBook:
hostname -I
# or
ifconfig | grep "inet "
...
server {
    ...
    location / {
        proxy_pass http://autonomyx-agentbook:5055;
        proxy_http_version 1.1;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
...
services:
  frontend:
    image: autonomyx/agentbook_frontend:v1-latest
    pull_policy: always
    environment:
      - API_URL=https://notebook.example.com
    ports:
  ...
  api:
    image: autonomyx/agentbook_api:v1-latest
    pull_policy: always
    environment:
      - OPEN_NOTEBOOK_ENCRYPTION_KEY=${OPEN_NOTEBOOK_ENCRYPTION_KEY}
    ports:
...
1. **Check API_URL is set**:
   ```bash
   docker exec autonomyx-agentbook env | grep API_URL
   ```
...
2. Check container logs: `docker logs autonomyx-agentbook`
...
**Cause:** Your reverse proxy has a default timeout (often 30s) that's shorter than Autonomyx AgentBook's operations.
...
```bash
docker logs autonomyx-agentbook
...
**Version Requirement:**
- **Autonomyx AgentBook v1.3.2+** is required for file uploads >10MB
...
**Note:** Autonomyx AgentBook's API includes CORS headers in error responses, but this only works for errors that reach the application. Proxy-level errors (like 413 from nginx) need to be configured at the proxy level.
...
3. **Reverse proxy not forwarding `/api/*` correctly**:
   ...
   location /api/ {
       proxy_pass http://autonomyx-agentbook:5055/api/;  # Note the trailing slash!
   }
   ```
...
9. **Monitor logs** regularly: `docker logs autonomyx-agentbook`
...
If you're running Autonomyx AgentBook **version 1.0.x or earlier**, you may need to use the legacy two-port configuration where you explicitly route `/api/*` to port 5055.

**Check your version:**
```bash
docker exec autonomyx-agentbook cat /app/package.json | grep version
```
